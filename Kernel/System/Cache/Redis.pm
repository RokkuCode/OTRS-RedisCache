...
# Copyright (C) 2020 by Holger Schramm <dev@strace.it>
# --
...
our @ObjectDependencies = ( 'Kernel::Config', 'Kernel::System::Log', 'Kernel::System::Main', 'Kernel::System::Storable', );
...
sub new
{
    ...
    $Self->{Config} = {
        Address        => $ConfigObject->Get( 'Cache::Redis' )->{ServerTCP}      || '127.0.0.1:6379',
        Socket         => $ConfigObject->Get( 'Cache::Redis' )->{ServerSocket}   || '/var/run/redis/redis-server.sock',
...
sub Get
{
    ...
    my $StorableObject = $Kernel::OM->Get( 'Kernel::System::Storable' );
    my $Data           = $StorableObject->Deserialize( Data => $Value, );
...
sub Delete
{
    ...
    eval { $Result = $Self->{Redis}->del( $Key ); };
...
sub CleanUp
{
    ...
    eval {
        ...
        {
            my $KeepTypesRegex =
                join( '|', map { "\Q$_\E" } @{ $Param{KeepTypes} } );
        ...
        for my $Type ( @TypeList )
        {
            ...
            {
                ( $Cursor, $KeysRef ) =
                    $Self->{Redis}->scan( $Cursor, MATCH => "$Type:*" );
...
sub _Connect
{
    ...
    my $Loaded     = $MainObject->Require( $Self->{Config}{RedisFast} ? 'Redis::Fast' : 'Redis', );
    return if !$Loaded;
    ...
    eval {
        if ( $Self->{Config}{RedisFast} )
        {
            ...
            {
                $Self->{Redis} =
                    Redis::Fast->new( socket => $Self->{Config}{Socket} );
            }
            else
            {
                $Self->{Redis} =
                    Redis::Fast->new( server => $Self->{Config}{Address} );
            }
        ...
        else
        {
            if ( $use_socket )
            {
                $Self->{Redis} = Redis->new( socket => $Self->{Config}{Socket} );
            }
            else
            {
